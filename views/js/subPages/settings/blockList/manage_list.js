...
const membCardsChoosed = document.getElementsByClassName(
  "manager__card--right"
);
  ...
  Array.prototype.forEach.call(membCardBTNS, (btn) => {
    btn.addEventListener("click", (event) => {
      ...
      if (btn.classList[1] === "right") {
        classes = `left userID_${userID} userNum_${membCards.length + 1}`;
        if (num > 0) num--;
      ...
      if (btn.classList[1] === "left") {
        num++;
        classes = `right userID_${userID} userNum_${num}`;
...
submitBTN.addEventListener("click", async (event) => {
  ...
  if (IDs.length > 0) {
    ...
    const userIDs = [];
    Array.prototype.forEach.call(membCardsChoosed, (membCardChoosed) => {
      userIDs.push(membCardChoosed.classList[2].split("_")[1]);
    });
    ...
    const response = await fetch(
      `http://127.0.0.1:3000/api/v1/users/2/blockList/deleteUser`,
      requestOptions
    );
    ...
    if (result.status === "success") {
      window.alert("Done");
      location.replace(
        `http://127.0.0.1:3000/userPage/${
          document.getElementsByClassName("body")[0].classList[1].split("_")[1]
        }/informations/block_list`
      );
    } else {
