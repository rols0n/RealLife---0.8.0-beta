  ...
  Array.prototype.forEach.call(membCardBTNS, (btn) => {
    btn.addEventListener("click", (event) => {
      ...
      if (btn.classList[1] === "right") {
        classes = `left userID_${userID} friendNum_${membCards.length + 1}`;
        if (num > 0) num--;
      ...
      if (btn.classList[1] === "left") {
        num++;
        classes = `right userID_${userID} friendNum_${num}`;
  ...
  if (IDs.length > 0)
    ...
    Array.prototype.forEach.call(IDs, async (id) => {
      ...
      const response = await fetch(
        `http://127.0.0.1:3000/api/v1/users/deleteFriend/${id}`,
        requestOptions
      );
      ...
      if (result.status === "success") {
        window.alert("Done");
        location.replace(
          `http://127.0.0.1:3000/userPage/639899d08e4daa1c649ed2e5/informations/manage_friends/delete_friend`
        );
      } else {
