  ...
  Array.prototype.forEach.call(membCardBTNS, (btn) => {
    btn.addEventListener("click", (event) => {
      ...
      if (btn.classList[1] === "right") {
        classes = `left userID_${userID} adminNum_${membCards.length + 1}`;
        if (num > 0) num--;
      ...
      if (btn.classList[1] === "left") {
        num++;
        classes = `right userID_${userID} adminNum_${num}`;
  ...
  if (IDs.length > 0)
    ...
    Array.prototype.forEach.call(IDs, async (id) => {
      ...
      const response = await fetch(
        `http://127.0.0.1:3000/api/v1/groups/${groupID}/administrators`,
        requestOptions
      );
